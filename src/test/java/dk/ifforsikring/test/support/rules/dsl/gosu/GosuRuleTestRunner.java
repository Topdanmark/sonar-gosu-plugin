...
import dk.ifforsikring.sonarqube.gosu.plugin.GosuFileParser;
import dk.ifforsikring.sonarqube.gosu.plugin.rules.BaseGosuRule;
import dk.ifforsikring.test.support.GosuSensorContextTester;
import dk.ifforsikring.test.support.rules.dsl.specification.RuleRunner;
...
final class GosuRuleTestRunner implements RuleRunner<List<Issue>> {
    ...
    public List<Issue> executeRule() {
        ...
        final List<dk.ifforsikring.sonarqube.gosu.plugin.issues.Issue> issues = analyse(gosuInputFile, context);
    ...
    private List<dk.ifforsikring.sonarqube.gosu.plugin.issues.Issue> analyse(InputFile inputFile, SensorContextTester context) {
        try {
